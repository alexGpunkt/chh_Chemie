...
/* ============================================================
   ...
   Aufruf:  node werkzeuge/uebungsblatt-pruefen.js [FC|PS|GA|WA|SZ|ME|QB|SL|KW|AL|OS|ES]
...
const { werteAus, fuelle, baue, fuerStufe } = require('../assets/js/ausdruck.js');
...
function pruefeGenerator(bereich, einheit, gen) {
  ...
  /* Jede Stufe wird einzeln durchgerechnet. Ein enger gezogener
     Zahlenbereich kann eine Bedingung unerfüllbar machen oder eine
     krumme Lösung erzeugen — beides fiele sonst erst auf dem
     ausgedruckten Blatt auf, und dort lässt es sich nicht mehr ändern.
     Geprüft werden nur die Stufen, auf denen der Generator auch steht. */
  const pfade = Array.isArray(gen.pfade) ? gen.pfade : ['A', 'B', 'C'];
  for (const stufe of pfade) pruefeStufe(wo, gen, stufe);
}

function pruefeStufe(basis, gen, stufe) {
  const wo = `${basis} [${stufe}]`;
  const wirksam = fuerStufe(gen, stufe);
  const stellen = wirksam.round ?? 2;
  let gebaut = 0;
  const gesehen = new Set();

  for (let i = 0; i < PROBEN; i++) {
    let a;
    try { a = baue(gen, stufe); }
    catch (e) {
    ...
    if (Number.isFinite(roh)) {
      if (wirksam.gerundet) {
        ...
        if (!/[Rr]unde/.test(wirksam.template)) {
    ...
    if (a.answer < 0 && NIE_NEGATIV.includes(wirksam.unit_label)) {
      fehler.push(`${wo}: negatives Ergebnis ${a.answer} ${wirksam.unit_label} bei „${a.prompt}"`);
      return;
    }
...
console.log(`${einheiten} Einheiten, ${generatoren} Generatoren, je Lernweg ${PROBEN} Proben gerechnet.`);
if (hinweis.length) {
