...
/* ============================================================
   ...
   Aufruf:  node werkzeuge/uebungsblaetter.js            (alle 76)
            node werkzeuge/uebungsblaetter.js qb-03      (nur eines)
...
/* ---------- Abschlussaufgabe ----------
   Vier Rechenaufgaben allein sind kein differenziertes Blatt: Bis hierher
   unterscheiden sich A und C nur in den Zahlen, alle drei Blätter fragen
   dasselbe ab, nämlich AFB I/II. Das Lehrbuch stuft dagegen über den
   Operator: nenne/beschreibe — erkläre/vergleiche — begründe/beurteile.

   Die Formulierungen stehen hier als Vorlage und nicht in den Daten, weil
   sie sich auf DAS BLATT beziehen, nicht auf das Thema: Sie fragen nach
   dem eigenen Rechenweg, und den gibt es in jeder Einheit. Der Titel der
   Einheit wird eingesetzt, damit die Aufgabe nicht im Leeren steht.
   Fachliche Vertiefungsfragen zum Inhalt stehen weiter im digitalen Teil
   und in der Lehrbuchkarte — hier geht es um das, was das Papier besser
   kann als der Bildschirm: den Weg aufschreiben und begründen. */
const ABSCHLUSS = {
  A: t => 'Beschreibe mit eigenen Worten, was du auf diesem Blatt gerechnet hast. '
    + `Nenne die Formel, die du für „${t}" gebraucht hast, und schreibe auf, wofür jeder Buchstabe steht.`,
  B: t => 'Erkläre einer Mitschülerin, die gefehlt hat, wie du bei Aufgabe 2 vorgegangen bist. '
    + `Schreibe in ganzen Sätzen und ordne die Aufgabe dem Thema „${t}" zu.`,
  C: t => 'Prüfe zwei deiner Ergebnisse auf Plausibilität und begründe jeweils in einem Satz, '
    + 'warum die Größenordnung stimmen kann. Beurteile anschließend, welche Aufgabe dieses Blattes '
...
function blattBauen(bereich, einheitId, blatt, einheitTitel, stufe, lehrbuch) {
  ...
  if (lehrbuch && lehrbuch.kapitel) {
    doc.text(`Im Lehrbuch: Kapitel ${lehrbuch.kapitel} — ${lehrbuch.titel}, S. `
      ...
      { groesse: 9.5, abstand: 12, farbe: [0.35, 0.35, 0.35] });
  }
  ...
  /* ---------- Abschlussaufgabe ----------
     Steht vor der Selbstkontrolle, weil sie zu den Aufgaben gehört, und ist
     eigens beschriftet, damit niemand im Lösungskasten nach ihr sucht: Auf
     sie gibt es keine Zahl als Antwort. */
  doc.platzPruefen(170);
  doc.luecke(4);
  doc.text(`Zum Schluss — schriftlich, ohne Rechnung`, { groesse: 11, fett: true, abstand: 3 });
  doc.text(ABSCHLUSS[stufe](einheitTitel || 'dieses Thema'), { groesse: 11, abstand: 4 });
  doc.luecke(4);
  doc.schreibfeld(stufe === 'C' ? 120 : 96);
  doc.luecke(16);
