...
const nachladeZuschlag = Math.max(0, ...['assets/js/animationen-7.js',
  'assets/js/animationen-8.js', 'assets/js/animationen-9.js']
  .filter(f => fs.existsSync(P(f))).map(f => gzip(P(f))));
...
for (const seite of seiten) {
  ...
  /* animationen.js laedt seinen Fachteil selbst nach. Ohne diesen Zuschlag
     wuerde die Aufteilung die Messung schoenen statt die Seite zu entlasten:
     gezaehlt wird der groesste Fachteil, also der ungünstigste Fall. */
  if (/animationen\.js/.test(html)) summe += nachladeZuschlag;
  ...
  if (/pruefung\.js/.test(html) && !/engine\.js/.test(html) && fs.existsSync(P('assets/js/engine.js'))) {
    summe += gzip(P('assets/js/engine.js'));
  }
...
/* ---------- 2 · JavaScript und CSS insgesamt ----------
   ...
   lässt: Gezählt wird die Vereinigung aus
     ...
     (b) allem, was ein <script src> oder <link href> einer Seite im
         Hauptverzeichnis anfordert.

   Damit fallen zwei Gruppen heraus, und zwar von selbst:

     dashboard/*     Das Lehrerdashboard und die Beameransicht stehen
                     seit v3 nicht mehr im Offlinepaket und werden von
                     keiner Schülerseite eingebunden. Sie laden weiterhin
                     normal über das Netz und sind offline ohnehin
                     nutzlos — es sind Live-Ansichten auf Supabase.
     dev-tools.js    Wird von dev-boot.js nur bei devMode nachgeladen.
                     Auf master fordert es niemand an.

   Wer eine dieser Dateien wieder in sw.js einträgt oder in eine Seite
   einbindet, bekommt sie hier automatisch zurück in die Rechnung. */

const swVorabliste = [...fs.readFileSync(P('sw.js'), 'utf8')
  .matchAll(/'([^']+\.(?:html|css|js|json))'/g)].map(m => m[1]);
...
const animQuelle = ['assets/js/animationen.js', 'assets/js/animationen-7.js',
  'assets/js/animationen-8.js', 'assets/js/animationen-9.js']
  .filter(f => fs.existsSync(P(f))).map(f => fs.readFileSync(P(f), 'utf8')).join(String.fromCharCode(10));
...
zeilen.push(`  · ${schleifen} rAF-Aufrufe in den Animationsdateien, ${schleifenNamen.size} unterschiedliche Rückruffunktionen`);
