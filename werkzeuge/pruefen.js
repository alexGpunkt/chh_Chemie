...
try {
  ...
  /* Der Sollumfang steht in units/index.json, nicht hier. Eine feste Zahl
     im Prüfer wäre eine zweite Wahrheit, die beim nächsten Themenfeld
     vergessen wird. Geprüft wird deshalb nur, dass Verzeichnis und Dateien
     deckungsgleich sind — und dass überhaupt etwas da ist. */
  if (!verzeichnet.size) fehler.push('units/index.json enthält keine Einheiten');
  if (verzeichnet.size !== vorhanden.size) {
    fehler.push(`Einheitenumfang: Verzeichnis ${verzeichnet.size}, Dateien ${vorhanden.size} — das muss übereinstimmen`);
  }
...
const animDateien = ['assets/js/animationen.js', 'assets/js/animationen-7.js',
  'assets/js/animationen-8.js', 'assets/js/animationen-9.js'];
const animQuelle = animDateien.map(lies).join(String.fromCharCode(10));
...
/* Die Fachteile liegen in eigenen Dateien und werden erst beim Zugriff
   nachgeladen. Welche Animation in welcher Datei steht, weiß animationen.js
   aus der Tabelle MODUL. Sie ist eine zweite Wahrheit neben der
   Registrierung — also wird sie hier gegen sie gehalten. */
const modulBlock = (lies('assets/js/animationen.js').match(/const MODUL = \{([\s\S]*?)\n  \};/) || [])[1] || '';
const inTabelle = new Set([...modulBlock.matchAll(/([a-z0-9]+)\s*:\s*\d/g)].map(m => m[1]));
for (const id of bekannteAnim) {
  if (!inTabelle.has(id)) fehler.push(`Animation „${id}" fehlt in der Tabelle MODUL in animationen.js — sie wird nie nachgeladen`);
}
for (const id of inTabelle) {
  if (!bekannteAnim.has(id)) fehler.push(`Tabelle MODUL nennt „${id}", aber keine Datei registriert diese Animation`);
}
melde(`${inTabelle.size} Einträge in MODUL decken sich mit den Registrierungen`);

const fragenBlock = (lies('assets/js/animationen.js').match(/const FRAGEN = \{([\s\S]*?)\n  \};/) || [])[1] || '';
const fragenIds = [...fragenBlock.matchAll(/^ {4}([a-z0-9]+):/gm)].map(m => m[1]);
const fragenVerwaist = fragenIds.filter(id => !bekannteAnim.has(id));
if (fragenVerwaist.length) {
  warnung.push(`FRAGEN nennt ${fragenVerwaist.length} Animationen, die es nicht gibt: ${fragenVerwaist.join(', ')}`);
}
melde(`${fragenIds.length - fragenVerwaist.length} Vorhersagefragen, alle einer Animation zugeordnet`);
...
const NICHT_CACHEN = {
  ...
  'dev-tools': 'Entwicklermenü. Wird seit v3 von dev-boot.js nur bei devMode '
...
{
  /* Geprüft werden die Seiten, die im Browser eines Kindes landen können.
     werkzeuge/ gehört nicht dazu: Dort liegt die Druckvorlage der
     Einrichtungsanleitung, die nie ausgeliefert wird. Eine Richtlinie
     hineinzuschreiben, damit die Prüfung schweigt, wäre eine Attrappe. */
  const seiten = alleDateien('.', '.html').filter(f => !f.startsWith('werkzeuge/'));
...
/* ---------- 10 · externe Übungen ----------
   Freigegebene Plattform, gültige Adresse, keine Dublette innerhalb einer
   Einheit — und je Einheit mindestens ein Verweis, der auf dem Basisweg
   sichtbar ist. Eine Sammlung ist auf B und C beschränkt; bestünde eine
   Einheit nur aus Sammlungen, stünde auf dem Basisweg eine leere Karte.

   Ob die Adresse noch lebt, prüft dieses Werkzeug NICHT: Das braucht Netz
   und gehört nicht in einen Lauf, der auch offline durchgehen muss. Dafür
   ist werkzeuge/uebungslinks_einspielen.py da, das jede Kennung beim
   Eintragen gegen die Plattform hält. */
const ERLAUBT = ['learningapps.org', 'serlo.org', 'allgemeinbildung.ch', 'schlaukopf.de',
                 'h5p.org', 'schule-bw.de', 'learningsnacks.de', 'quizlet.com', 'zum.de'];
/* engine.js liest die Plattform aus der Adresse, nicht aus dem Feld
   „quelle". Das Feld ist damit für die Anzeige überflüssig — es steht in
   den Daten, damit ein Mensch sie lesen kann. Hier wird beides gegen-
   einander gehalten; sonst wäre die Doppelung nur eine Fehlerquelle. */
const QUELLE_ZU_HOST = {
  LearningApps: 'learningapps.org',
  Serlo: 'serlo.org',
  'allgemeinbildung.ch': 'allgemeinbildung.ch',
  Schlaukopf: 'schlaukopf.de',
  H5P: 'h5p.org',
  Learningsnacks: 'learningsnacks.de',
  Quizlet: 'quizlet.com',
  ZUM: 'zum.de'
};
let links = 0;
...
for (const f of einheiten) {
  ...
  for (const l of liste) {
    ...
    const sollHost = QUELLE_ZU_HOST[l.quelle];
    if (sollHost && !(host === sollHost || host.endsWith('.' + sollHost))) {
      fehler.push(`${f}: ${l.url} ist als „${l.quelle}" eingetragen, liegt aber auf ${host}`);
    }
    if (gesehen.has(l.url)) fehler.push(`${f}: ${l.url} ist doppelt eingetragen`);
    gesehen.add(l.url);
    if (!l.pfade || l.pfade.includes('A')) aufBasisweg++;

    if (!proUebung.has(l.url)) proUebung.set(l.url, []);
    proUebung.get(l.url).push(d.unit);
  }
  if (!liste.length) {
    fehler.push(`${f}: keine externe Übung hinterlegt`);
  } else if (!aufBasisweg) {
    fehler.push(`${f}: alle Übungen sind auf B/C beschränkt — der Basisweg sähe eine leere Karte`);
  }
}
/* Dieselbe Übung in vielen Einheiten heißt meist: Sie passt zu keiner davon
   genau. Bei Grundlagen wie den zwischenmolekularen Kräften ist die
   Mehrfachnutzung richtig — deshalb Hinweis, kein Fehler. */
for (const [url, orte] of proUebung) {
  if (orte.length > 3) warnung.push(`Übung ${url} ist in ${orte.length} Einheiten verlinkt: ${orte.join(', ')}`);
}
melde(`${links} externe Übungsverweise in ${proUebung.size} Adressen geprüft`);
   ...
   stehen — sonst ist unklar, woher sie stammen. */
{
  ...
  for (const f of einheiten) {
    ...
    for (const video of d.videos || []) {
      ...
      if (video.stufe && !pfade.has(video.stufe)) {
        fehler.push(`${f}: Video ${id} verlangt mindestens Lernweg ${video.stufe}, den es hier nicht gibt`);
      }
      stufen.add(video.stufe);

      /* Die Einbettadresse muss zur Laufadresse gehören. Zwei Videos, die
         versehentlich auf dieselbe Lumi-Kennung zeigen, wären in der
         Anwendung nicht zu unterscheiden — dasselbe Video liefe zweimal. */
      const kennung = String(video.lumi || '').split('/run/')[1] || '';
      const einbett = (String(video.embed || '').match(/\/run\/([^/]+)\/embed$/) || [])[1] || '';
      if (kennung && einbett && kennung !== einbett) {
        fehler.push(`${f}: Video ${id} — Lauf- und Einbettadresse zeigen auf verschiedene Lumi-Kennungen`);
      }
      if (kennung) {
        if (!proLumi.has(kennung)) proLumi.set(kennung, []);
        proLumi.get(kennung).push(`${d.unit}/${id}`);
      }
      if (video.dauer_s == null) {
        warnung.push(`${f}: Video ${id} hat keine Laufzeit — der 45-Minuten-Plan rechnet dann ohne Videozeit`);
      }

      if (!proVideo.has(id)) proVideo.set(id, []);
      proVideo.get(id).push(d.unit);
    }
    /* Ohne ein Video auf Stufe A sähe der Basisweg gar keines — und genau
       dort hilft eine zweite Erklärung am meisten. */
    if ((d.videos || []).length && !stufen.has('A')) {
      fehler.push(`${f}: kein Video für den Basisweg (kein Eintrag mit stufe "A")`);
    }
  }

  for (const [kennung, orte] of proLumi) {
    const verschieden = new Set(orte.map(o => o.split('/')[1]));
    if (verschieden.size > 1) {
      fehler.push(`Lumi-Kennung ${kennung} steht für verschiedene Filme: ${orte.join(', ')}`);
    }
  }
  ...
  const quelldatei = 'videos-quellen.csv';
...
/* ---------- 10c · 45-Minuten-Plan und Hefteintrag ----------
   Die Vorgabe lautet: eine Einheit ohne Warm-up in etwa 45 Minuten. Ein
   Plan, dessen Summe nicht stimmt, ist schlimmer als keiner — er sieht aus
   wie eine Zusage. Geprüft wird deshalb die Summe, nicht die Aufteilung.

   Beim Hefteintrag geht es um dasselbe Versprechen von der anderen Seite:
   Am Ende der Reihe soll ein vollständiges Skript auf Papier liegen. Fehlt
   in einer Einheit eine Stufe, hat ein Kind dieses Lernwegs am Ende eine
   Lücke im Heft, die niemand bemerkt. */
{
  let plaene = 0, eintraege = 0;
  const noetig = ['merksatz', 'fachwoerter'];
  for (const f of einheiten) {
    const d = geladen.get(f);
    const pfade = [...new Set((d.tasks || []).map(t => t.path))].sort();

    const zp = d.zeitplan;
    if (!zp) { fehler.push(`${f}: kein Zeitplan hinterlegt`); }
    else {
      plaene++;
      for (const p of pfade) {
        const schritte = zp[p];
        if (!schritte || typeof schritte !== 'object') {
          fehler.push(`${f}: Zeitplan für Lernweg ${p} fehlt`);
          continue;
        }
        const summe = Object.values(schritte).reduce((s, x) => s + (Number(x) || 0), 0);
        if (summe !== zp.gesamt_min) {
          fehler.push(`${f}: Zeitplan ${p} ergibt ${summe} statt ${zp.gesamt_min} Minuten`);
        }
        if (!schritte.mitschrift) {
          fehler.push(`${f}: im Zeitplan ${p} ist keine Zeit für den Hefteintrag vorgesehen`);
        }
        if (schritte.aufgaben && schritte.aufgaben < 12) {
          warnung.push(`${f}: Zeitplan ${p} lässt nur ${schritte.aufgaben} Minuten für die Aufgaben`);
        }
        if ((d.videos || []).length && !schritte.video) {
          warnung.push(`${f}: Zeitplan ${p} sieht keine Videozeit vor, obwohl Videos hinterlegt sind`);
        }
      }
    }

    const ms = d.mitschrift;
    if (!ms) { fehler.push(`${f}: kein Hefteintrag hinterlegt`); continue; }
    for (const p of pfade) {
      const stufe = ms[p];
      if (!stufe || !Array.isArray(stufe.punkte) || !stufe.punkte.length) {
        fehler.push(`${f}: Hefteintrag für Lernweg ${p} fehlt — dieses Kind hätte am Ende eine Lücke im Skript`);
        continue;
      }
      eintraege++;
      const arten = new Set(stufe.punkte.map(x => x.art));
      for (const a of noetig) {
        if (!arten.has(a)) warnung.push(`${f}: Hefteintrag ${p} ohne Punkt der Art "${a}"`);
      }

      /* Die Punkte sind Verweise. Zeigt einer ins Leere, fehlt im Heft
         genau der Punkt, den die Zeilenzahl noch verspricht — deshalb wird
         jede Quelle gegen die Einheit gehalten. */
      const lk = (d.lernkarten || {})[p] || {};
      const fk = d.formelkarte || {};
      const quelle = {
        merksatz: lk.merke,
        fachwoerter: (d.wortspeicher || []).length ? d.wortspeicher : null,
        beispiel: lk.beispiel && (lk.beispiel.aufgabe || (lk.beispiel.schritte || []).length),
        eigene_worte: (lk.erklaerung || []).length ? lk.erklaerung : null,
        regel: (fk.saetze || []).length ? fk.saetze : null,
        formeln: (fk.formeln || []).length ? fk.formeln : null,
        pruefsatz: (d.can_do || {})[p]
      };
      for (const punkt of stufe.punkte) {
        if (!quelle[punkt.art]) {
          fehler.push(`${f}: Hefteintrag ${p} verweist auf "${punkt.art}", aber diese Quelle ist in der Einheit leer`);
        }
      }
    }
    /* Der Umfang muss mit dem Lernweg wachsen — sonst schreibt der
       Vertiefungsweg dasselbe wie der Basisweg und die Kürzung ist keine.
       Gemessen an der Zahl der Punkte: Die tatsächlichen Zeilen zählt
       engine.js beim Anzeigen aus, hier stehen nur die Verweise. */
    const anzahl = ['A', 'B', 'C'].map(p => (ms[p]?.punkte || []).length);
    if (anzahl.every(Boolean) && !(anzahl[0] <= anzahl[1] && anzahl[1] <= anzahl[2])) {
      warnung.push(`${f}: Hefteintrag wird zum höheren Lernweg hin nicht umfangreicher (${anzahl.join(' / ')} Punkte)`);
    }
  }
  melde(`${plaene} Zeitpläne über 45 Minuten und ${eintraege} Hefteinträge geprüft`);
}
...
try {
  ...
  /* Die Zahl steht in units/index.json. Zwei Wahrheiten wären eine zu viel. */
  const sollEinheiten = (liesJson('units/index.json').bereiche || [])
    .reduce((n, b) => n + (b.einheiten || []).filter(e => !e.geplant).length, 0);
  if (Number(manifest.einheiten) !== sollEinheiten) {
    fehler.push(`version.json: einheiten ist ${manifest.einheiten}, units/index.json zählt ${sollEinheiten}`);
  }
...
try {
  ...
  for (const name of ['chemie710_validate_student_login', 'chemie710_aufraeumen', 'chemie710_person_export', 'chemie710_person_loeschen', 'chemie710_student_anmelden', 'chemie710_student_sitzung', 'chemie710_ist_lehrkraft', 'chemie710_wartung_status', 'chemie710_lehrkraft_sperren', 'chemie710_lehrkraft_uebersicht']) {
  ...
  for (const name of ['chemie710_aufraeumen', 'chemie710_person_export', 'chemie710_person_loeschen']) {
  ...
  if (!/delete\s+from\s+public\.chemie710_student_tokens\s+where\s+gueltig_bis\s*</i.test(sql)) {
  ...
  for (const tabelle of ['chemie710_wartung_laeufe', 'chemie710_teacher_audit']) {
  ...
  if (!/insert\s+into\s+public\.chemie710_wartung_laeufe/i.test(sql)) {
    fehler.push('supabase/setup.sql: chemie710_aufraeumen protokolliert seine Läufe nicht');
  }
  if (!/insert\s+into\s+public\.chemie710_teacher_audit/i.test(sql)) {
  ...
  for (const name of ['chemie710_lernmodus', 'chemie710_lernzeit_melden', 'chemie710_unterricht_setzen', 'chemie710_freigeben', 'chemie710_freigabe_zuruecknehmen']) {
    if (!new RegExp('create\\s+or\\s+replace\\s+function\\s+public\\.' + name + '\\b', 'i').test(sql)) {
      fehler.push(`supabase/setup.sql: Funktion ${name} fehlt`);
    }
  }
  for (const tabelle of ['chemie710_unterricht', 'chemie710_freigaben', 'chemie710_lernzeit']) {
    if (!new RegExp('create\\s+table\\s+if\\s+not\\s+exists\\s+public\\.' + tabelle + '\\b', 'i').test(sql)) {
  ...
  if (!/gilt_bis\s+is\s+null\s+or\s+aktuell\.gilt_bis\s*>\s*now\(\)/i.test(sql)) {
    fehler.push('supabase/setup.sql: chemie710_lernmodus lässt den Bewertungsmodus nicht ablaufen');
  }
  /* Lernzeit ist personenbezogen und gehört in Auskunft und Löschfrist. */
  if (!/'lernzeit'/.test(sql)) {
    fehler.push('supabase/setup.sql: chemie710_person_export enthält die Lernzeit nicht');
  }
  if (!/delete\s+from\s+public\.chemie710_lernzeit/i.test(sql)) {
    fehler.push('supabase/setup.sql: Lernzeiten werden nicht aufgeräumt');
  }
  /* Vorbereitung des Abgleichs: Der nächste Schritt vergleicht ein
     bestehendes Supabase-Projekt gegen den vollständigen Sollstand. Darum
     wird hier nicht nur das zuletzt Ergänzte geprüft, sondern das ganze
     serverseitige Inventar, das der Browser voraussetzt. */
  const sollTabellen = [
    'chemie710_students', 'chemie710_events', 'chemie710_progress', 'chemie710_wartung_laeufe',
    'chemie710_teachers', 'chemie710_teacher_audit', 'chemie710_student_tokens',
    'chemie710_unterricht', 'chemie710_freigaben', 'chemie710_lernzeit'
  ];
  for (const tabelle of sollTabellen) {
    if (!new RegExp('create\\s+table\\s+if\\s+not\\s+exists\\s+public\\.' + tabelle + '\\b', 'i').test(sql)) {
      fehler.push(`supabase/setup.sql: Solltabelle ${tabelle} fehlt`);
    }
    if (!new RegExp('alter\\s+table\\s+public\\.' + tabelle + '\\s+enable\\s+row\\s+level\\s+security', 'i').test(sql)) {
      fehler.push(`supabase/setup.sql: Solltabelle ${tabelle} ohne Row Level Security`);
    }
  }
  const sollFunktionen = [
    'chemie710_validate_student_login', 'chemie710_aufraeumen', 'chemie710_wartung_status',
    'chemie710_person_export', 'chemie710_person_loeschen', 'chemie710_lehrkraft_freischalten',
    'chemie710_lehrkraft_sperren', 'chemie710_ist_lehrkraft', 'chemie710_lehrkraft_uebersicht',
    'chemie710_student_anmelden', 'chemie710_token_student', 'chemie710_student_sitzung',
    'chemie710_student_abmelden', 'chemie710_lernmodus', 'chemie710_lernzeit_melden',
    'chemie710_unterricht_setzen', 'chemie710_freigeben', 'chemie710_freigabe_zuruecknehmen'
  ];
  for (const name of sollFunktionen) {
    if (!new RegExp('create\\s+or\\s+replace\\s+function\\s+public\\.' + name + '\\b', 'i').test(sql)) {
      fehler.push(`supabase/setup.sql: Sollfunktion ${name} fehlt`);
    }
  }
  if (!/revoke\s+all\s+on\s+function\s+public\.chemie710_validate_student_login\s*\(\s*text\s*,\s*text\s*\)\s+from\s+public\s*,\s*anon\s*,\s*authenticated/i.test(sql)) {
    fehler.push('supabase/setup.sql: alter Loginpfad chemie710_validate_student_login ist am Ende nicht für public/anon/authenticated entzogen');
  }
...
try {
  ...
  for (const tabelle of ['chemie710_students', 'chemie710_events', 'chemie710_progress', 'chemie710_wartung_laeufe',
    'chemie710_teachers', 'chemie710_teacher_audit', 'chemie710_student_tokens', 'chemie710_unterricht',
    'chemie710_freigaben', 'chemie710_lernzeit']) {
   ...
   Unterrichts fällt genau dort aus, wo niemand es merkt. */
{
  ...
  /* Seit v3 liegt je Einheit ein Blatt für jeden Lernweg vor. Geprüft wird
     jedes einzeln: Ein fehlendes A-Blatt fiele sonst erst auf, wenn ein
     Kind auf dem Basisweg den Verweis anklickt. */
  let pdfs = 0;
  for (const f of einheiten) {
    ...
    const alt = f.replace('tasks.json', 'uebungsblatt.pdf');
    if (fs.existsSync(P(alt))) {
      fehler.push(`${alt} ist übrig geblieben — vor v3 gab es nur ein Blatt je Einheit; die Datei löschen`);
    }
  }
