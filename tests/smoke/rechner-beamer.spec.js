...
test.beforeEach(async ({ page }) => {
  await page.addInitScript(() => {
    localStorage.setItem('chemie710.dev.skipLogin', 'true');
    localStorage.setItem('chemie710.dev.supabaseDisabled', 'true');
    localStorage.setItem('chemie710.dev.trackerDisabled', 'true');
    localStorage.setItem('chemie710.dev.student', 'test1');
  });
});

test.describe('Taschenrechner', () => {
  test('rechnet und übernimmt das Ergebnis ins Antwortfeld', async ({ page }) => {
    await page.goto('/einheit.html?u=qb-03&p=B');
  ...
  test('blendet alles außer Aufgabe und Rechner aus, ohne zu scrollen', async ({ page }) => {
    await page.goto('/einheit.html?u=sl-02&p=B');
    await page.getByRole('button', { name: /Los geht/ }).click();
    await expect(page.locator('.frage')).toBeVisible();
...
test.describe('Entwicklermenü wird nachgeladen', () => {
  ...
  test('Einstellungen gelten sofort, das Menü kommt nach', async ({ page }) => {
    ...
    const sofort = await page.evaluate(() => ({
      student: window.CHEMIE710_STUDENT?.login_name || null,
      helfer: typeof window.Chemie710DevTools?.saveTestStudent === 'function',
      loginUebersprungen: window.CHEMIE710_SUPABASE?.skipStudentLogin === true
    }));
...
test.describe('Beameransicht', () => {
  ...
  async function senden(page, zeilen, einstellungen = {}) {
    await page.evaluate(([z, e]) => {
      const kanal = new BroadcastChannel('chemie710-beamer');
      kanal.postMessage({
        typ: 'daten',
        ts: Date.now(),
        klasse: '9a',
        reihe: ['qb-01', 'qb-02', 'qb-03', 'qb-04'],
  ...
  test('setzt das Strichmännchen nach dem Fortschritt in der Reihe', async ({ page }) => {
    ...
    /* Reihe mit vier Einheiten, Kind steht in qb-03 bei 50 %:
       (2 + 0,5) / 4 = 62,5 % der Gesamtstrecke. */
    await senden(page, [
      { id: 'a', name: 'Anna A.', unit: 'qb-03', path: 'B', percent: 50,
    ...
    /* Umschalten auf die Einheit: dann zählt nur der Stand in qb-03. */
    await senden(page, [
      { id: 'a', name: 'Anna A.', unit: 'qb-03', path: 'B', percent: 50,
        letzterPing: new Date(jetzt - 2000).toISOString(), pingFehler: 0, pingTaktSekunden: 20 }
    ], { bezug: 'einheit' });
