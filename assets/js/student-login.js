/* ============================================================
   ...
   - Sichere Token-Anmeldung über chemie710_student_anmelden
...
(() => {
  ...
  const STORAGE_KEY = 'chemie710.student';
  const MAX_OFFLINE_AGE_MS = 7 * 24 * 60 * 60 * 1000;
  const TOKEN_CHECK_INTERVAL_MS = 15 * 60 * 1000;
  let resolveReady;
  let tokenRefreshPromise = null;

  window.CHEMIE710_STUDENT_READY = new Promise(resolve => {
  ...
  function saveStudent(student) {
    ...
    localStorage.setItem('chemie710.name', value.display_name);
    localStorage.setItem('chemie710.student_id', value.student_id);
    localStorage.setItem('chemie710.class_code', value.class_code);
    window.CHEMIE710_STUDENT = value;
    return value;
  }

  function clearStudent() {
    localStorage.removeItem(STORAGE_KEY);
    localStorage.removeItem('chemie710.name');
    localStorage.removeItem('chemie710.student_id');
    localStorage.removeItem('chemie710.class_code');
    delete window.CHEMIE710_STUDENT;
  }
  ...
  const TOKEN_KEY = 'chemie710.token';
  ...
  function tokenSpeichern(token, gueltigBis, studentId, checkedAt = Date.now()) {
    ...
    window.CHEMIE710_TOKEN = token;
  }

  function tokenLesen() {
    const wert = tokenInfoLesen();
    if (!wert) { delete window.CHEMIE710_TOKEN; return null; }
    window.CHEMIE710_TOKEN = wert.token;
    return wert.token;
  }

  function tokenLoeschen() {
    try { localStorage.removeItem(TOKEN_KEY); } catch { /* egal */ }
    delete window.CHEMIE710_TOKEN;
  }

  async function rpc(name, body = {}, token = null) {
    const response = await fetch(`${baseUrl()}/rest/v1/rpc/${name}`, {
      method: 'POST',
      headers: token ? { ...publicHeaders(), 'x-chemie710-token': token } : publicHeaders(),
  ...
  async function tokenEntwerten() {
    ...
    try { await rpc('chemie710_student_abmelden', {}, token); }
  ...
  async function validateStudent(loginName, classCode) {
    ...
    let row;
    try {
      row = await rpc('chemie710_student_anmelden', {
  ...
  async function tokenPruefen(token) {
    try {
      return await rpc('chemie710_student_sitzung', {}, token);
    } catch (error) {
      if (error.status === 404) {
  ...
  function addStyles() {
    if (document.querySelector('#chemie710-login-style')) return;
    const style = document.createElement('style');
    style.id = 'chemie710-login-style';
    style.textContent = `
  ...
  function showLogin(message = '') {
    ...
    form.addEventListener('submit', async event => {
      ...
      try {
        ...
        showUserChip(saved);
        resolveReady(saved);
        window.dispatchEvent(new CustomEvent('chemie710:student-ready', { detail: saved }));
      } catch (error) {
        errorBox.hidden = false;
  ...
  async function initialize() {
    ...
    if (CONFIG.devMode === true && CONFIG.skipStudentLogin === true) {
      const devStudent = window.Chemie710DevTools?.saveTestStudent(
      ...
      resolveReady(saved);
      window.dispatchEvent(new CustomEvent('chemie710:student-ready', { detail: saved }));
      console.info('[Chemie710 DEV] Schülerlogin wurde übersprungen.');
      return;
    }
    const stored = readStoredStudent();
    if (!stored) {
    ...
    if (!navigator.onLine) {
      if (verifiedAge <= MAX_OFFLINE_AGE_MS) {
        window.CHEMIE710_STUDENT = stored;
        showUserChip(stored);
        resolveReady(stored);
        return;
      }
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('chemie710:student-ready', { detail: verified }));
    } catch (error) {
  ...
  function lokaleLernstaendeLoeschen() {
    ...
    try {
      ...
      const eigene = [
        'chemie710.stand.' + kennung + '.',     // Bearbeitungsstände und „zuletzt“
        'chemie710.fehler.' + kennung,          // Fehlerprofil
        'chemie710.lesezeichen.' + kennung,     // Lesezeichen im Buchmodus
        'chemie710.lernmodus.' + kennung,       // zwischengespeicherte Unterrichtsfreigaben
        'chemie710.lernzeit.offen.' + kennung   // noch nicht übertragene Lernzeit je Einheit
      ];
      ...
      const geraeteweit = kennung === 'lokal'
        ? ['chemie710.stand.zuletzt', 'chemie710.fehler', 'chemie710.lesezeichen', 'chemie710.spiral', 'chemie710.pfad']
        : ['chemie710.spiral', 'chemie710.pfad'];
