...
const TRACKER_CONFIG = Object.assign(
  ...
  window.CHEMIE710_SUPABASE || {}
);

const Tracker = (() => {
  const QUEUE_KEY = 'chemie710.tracker.queue.v2';
  const SYNC_KEY = 'chemie710.tracker.sync';
  const DEVICE_KEY = 'chemie710.device_id';
  const SESSION_KEY = 'chemie710.session_id';
  const MAX_QUEUE = 500;
  ...
  /* ---------- Dauersendung des Fortschritts ----------
     Bisher ging eine Fortschrittszeile nur bei Pfadwahl, Wiederaufnahme
  ...
  function studentRecord() {
    try {
      const saved = JSON.parse(
        localStorage.getItem('chemie710.student') || 'null'
      );
  ...
  function headers(prefer = 'return=minimal') {
    ...
    const token =
      (window.Chemie710StudentLogin &&
        typeof window.Chemie710StudentLogin.token === 'function' &&
        window.Chemie710StudentLogin.token()) ||
      window.CHEMIE710_TOKEN ||
      null;

    if (token) {
      result['x-chemie710-token'] = token;
    }
  ...
  function enqueue(type, payload = {}) {
    ...
    if (!event.student_id) {
      console.warn(
        '[Chemie710 tracker] Kein gültiger Schüler angemeldet.'
      );
  ...
  async function schreibTokenSichern(force = false) {
    const login = window.Chemie710StudentLogin;
    if (!login || typeof login.ensureToken !== 'function') {
      return Boolean(headers()['x-chemie710-token']);
    }
  ...
  async function flush() {
    ...
    try {
      ...
      if ((response.status === 401 || response.status === 403) && await schreibTokenSichern(true)) {
        response = await fetch(base() + 'chemie710_events', {
      ...
      console.warn(
        '[Chemie710 tracker] Versand fehlgeschlagen:',
        error.message
      );
    } finally {
      sending = false;
  ...
  async function progress(snapshot = {}) {
    ...
    if (!student?.student_id) {
      console.warn(
        '[Chemie710 progress] Kein gültiger Schüler angemeldet.'
      );
    ...
    /*
     * Ohne Einheit gibt es nichts zu melden — chemie710_progress.unit ist
     ...
     * In chemie710_progress heißt dieses Feld "current_task".
     */
    delete row.task;
    ...
    try {
      ...
      const ziel = base() + 'chemie710_progress?' + query;
      const optionen = () => ({
        method: 'POST',
...
document.addEventListener(
  ...
  () => {
    const ready =
      window.CHEMIE710_STUDENT_READY ||
      Promise.resolve();
    ...
    ready
      ...
      .catch(error => {
        console.warn(
          '[Chemie710 tracker] Anmeldung nicht bereit:',
          error
        );
      });
  },
