/* ============================================================
   weiterlernen.js · Die beweglichen Kacheln der Startseite

   Zwei Dinge, die von der Liste der 76 Einheiten nicht zu leisten sind:

   1. „Weiterlernen" — wer eine Einheit angefangen hat, soll sie nicht in
      einer langen Liste wiederfinden müssen. Die Kachel führt direkt an
      die Stelle zurück, an der zuletzt gearbeitet wurde.
   2. Der Hinweis am Warm-up, dass es heute im Unterricht Pflicht ist.
      Ohne ihn erführe man das erst beim Öffnen einer Einheit.

   Braucht store.js (Stand). Ohne gespeicherten Stand erscheint keine
   Weiterlernen-Kachel — die Startseite bleibt dann genau wie vorher.
   ============================================================ */
(function () {
  'use strict';

  /* ---------- Warm-up heute: Pflicht oder Angebot? ----------
     Der Zustand kommt vom Server und trifft verspätet ein. Deshalb wird
     die Kachel nachträglich beschriftet und nicht auf sie gewartet:
     Eine Startseite, die erst nach der Netzantwort erscheint, ist keine. */
  (async function warmupKachelMarkieren() {
    const kachel = document.querySelector('a.eintrag[href^="warmup.html"]');
    if (!kachel || !window.Lernmodus) return;
    try {
      await Lernmodus.starten(null);
    } catch { return; }
    if (!Lernmodus.warmupNoetig?.()) return;

    kachel.classList.add('eintrag-pflicht');
    const nr = kachel.querySelector('.nr');
    if (nr) nr.textContent = 'Pflicht';
    const klein = kachel.querySelector('.txt span');
    if (klein) {
      klein.textContent = 'Heute im Unterricht: erst aufwärmen, dann die Einheit. Der Lauf zählt.';
    }
  })();
