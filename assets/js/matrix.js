/* ============================================================
   ...
   Datenschutz: Die Häkchen liegen unter chemie710.matrix.<Name> im
...
function bekannteNamen() {
  return Speicher.lies('chemie710.matrix.namen', []);
}
function namenMerken(n) {
  if (!n) return;
  const l = bekannteNamen();
  if (!l.includes(n)) { l.push(n); l.sort(); Speicher.schreib('chemie710.matrix.namen', l); }
  namenslisteBauen();
}
...
/* ---------- Häkchen ---------- */
function schluessel() { return 'chemie710.matrix.' + (M.name || '_ohne_namen'); }
...
function kopieren() {
  ...
  let t = `Kompetenzstand Chemie, JahrgÃ¤nge 7 bis 10 — ${M.name || 'ohne Namen'}\n`;
  t += `Stand: ${new Date().toLocaleDateString('de-DE')}\n`;
  t += `Grundlage: Rahmenlehrplan 1–10 Berlin/Brandenburg, Teil C Chemie\n\n`;
  for (const p of ['A', 'B', 'C']) {
