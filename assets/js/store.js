...
const LokalerStand = (() => {
  const FORMAT_SCHLUESSEL = 'chemie710.format';
  const FORMAT = 3;
  ...
  const MIGRATIONEN = {
    ...
    1() {
      const kennung = 'lokal';
      const paare = [
        ['chemie710.stand.zuletzt', 'chemie710.stand.' + kennung + '.zuletzt'],
        ['chemie710.fehler', 'chemie710.fehler.' + kennung],
        ['chemie710.lesezeichen', 'chemie710.lesezeichen.' + kennung]
      ];
      let umgezogen = 0;
    ...
    2() {
      const alt = Speicher.lies('chemie710.tracker.queue', null);
      Speicher.loesche('chemie710.tracker.queue');
  ...
  /* Ein einzelner unlesbarer Eintrag hat früher jede Funktion getroffen, die
     ...
     Bewusst eine Positivliste: Nicht alles unter chemie710.* ist JSON — die
  ...
  const JSON_PRAEFIXE = ['chemie710.stand.', 'chemie710.fehler', 'chemie710.lesezeichen', 'chemie710.tracker.queue'];
  /* Nur exakt: „chemie710.student_id" ist eine rohe UUID, „chemie710.student" JSON. */
  const JSON_GENAU = ['chemie710.student', 'chemie710.token', 'chemie710.spiral'];
  ...
  function starten() {
    ...
    if (!von) {
      const hatDaten = (() => {
        for (let i = 0; i < localStorage.length; i++) {
          if (String(localStorage.key(i) || '').startsWith('chemie710.')) return true;
        }
        return false;
      })();
    ...
    while (von < FORMAT) {
      ...
      try { bericht.schritte.push(von + '→' + (von + 1) + ': ' + schritt()); }
        ...
        console.warn('[Chemie710 Migration] Schritt', von, 'fehlgeschlagen:', e);
...
const Stand = (() => {
  const PRAEFIX = 'chemie710.stand.';
  const ZULETZT_ALT = 'chemie710.stand.zuletzt';
  ...
  function migriere(d, einheit) {
    ...
    while (von < FORMAT) {
      ...
      catch (e) { console.warn('[Chemie710 Stand] Migration', von, 'fehlgeschlagen:', e); return null; }
      von++;
    }
    if (stand !== d) {
...
function fehlerSchluessel() {
  ...
  return 'chemie710.fehler.' + kennung;
}
...
function fehlerDaten() {
  ...
  if (!p && key.endsWith('.lokal')) {
    p = Speicher.lies('chemie710.fehler', null);
    if (p) {
      Speicher.schreib(key, p);
      Speicher.loesche('chemie710.fehler');
    }
  }
  return p || {};
}
