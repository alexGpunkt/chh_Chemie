...
const Lernmodus = (() => {
  ...
  const CONFIG = window.CHEMIE710_SUPABASE || {};
  const ZUSTAND_PRAEFIX = 'chemie710.lernmodus.';
  const ZEIT_PRAEFIX = 'chemie710.lernzeit.offen.';
  ...
  function standardZustand() {
    return {
      modus: 'uebung', freigegeben: [], gilt_bis: null, lernzeit_heute: 0, geprueft: 0,
      /* Warm-up: Pflicht im Unterricht, freiwillig danach. „warmup_heute"
         heißt: Der gewertete Pflichtlauf von heute liegt vor. Der Standard
         ist bewusst „keine Pflicht" — wer offline arbeitet oder nicht
         angemeldet ist, soll nicht vor einer Sperre stehen, die niemand
         aufheben kann. */
      warmup_pflicht: false,
      warmup_heute: false,
      /* Wonach diese Person bewertet wird. Die Anwendung entscheidet das
         nicht, sie zeigt es nur an — festgelegt wird es im Dashboard. */
      bewertungsart: 'note'
    };
  }
  ...
  function student() {
    return window.Chemie710StudentLogin?.get?.() || window.CHEMIE710_STUDENT || null;
  }
  ...
  loesche('chemie710.lernmodus');
  loesche('chemie710.lernzeit.offen');
  ...
  function kopf() {
    ...
    const token = window.Chemie710StudentLogin?.token?.() || window.CHEMIE710_TOKEN || null;
    if (token) h['x-chemie710-token'] = token;
    return h;
  }

  async function rpc(name, koerper = {}, wiederholen = true) {
    if (window.Chemie710StudentLogin?.ensureToken) {
      await window.Chemie710StudentLogin.ensureToken({ force: false });
    }
    ...
    if (!antwort.ok) {
      if (wiederholen && (antwort.status === 401 || antwort.status === 403)
          && window.Chemie710StudentLogin?.ensureToken) {
        await window.Chemie710StudentLogin.ensureToken({ force: true });
  ...
  async function aktualisieren({ erzwingen = false } = {}) {
    ...
    try {
      const zeile = await rpc('chemie710_lernmodus');
      if (zeile) {
        zustand = {
          ...
          /* Ältere Datenbanken ohne die V31-Felder liefern hier undefined.
             Dann bleibt es beim offenen Zustand, statt eine Pflicht zu
             erfinden, die der Server gar nicht kennt. */
          warmup_pflicht: zeile.warmup_pflicht === true,
          warmup_heute: zeile.warmup_heute === true,
          bewertungsart: zeile.bewertungsart === 'fortschritt' ? 'fortschritt' : 'note',
          geprueft: Date.now()
        };
        schreib(zustandSchluessel(aktiveKennung), zustand);
        window.dispatchEvent(new CustomEvent('chemie710:lernmodus', { detail: { ...zustand } }));
      }
    } catch (fehler) {
      console.warn('[Chemie710 Lernmodus] nicht abrufbar:', fehler.message);
    }
    return zustand;
  }
  ...
  /* ---------- Warm-up ----------
     Im Unterricht ist das Warm-up Pflicht: Es steht vor der Einheit, und
     sein Ergebnis geht in die Bewertung ein. Außerhalb der Unterrichtszeit
     ist es ein Angebot.

     Die Prüfung, ob die Pflicht erfüllt ist, gehört auf den Server — ein
     Gerät, das sich selbst freigibt, gibt nichts frei. Diese Funktion sagt
     nur, was zuletzt vom Server kam. Ist gar nichts gekommen (kein Netz,
     keine Anmeldung, ältere Datenbank), lautet die Antwort „nicht nötig":
     Eine Sperre, die niemand aufheben kann, ist schlimmer als ein
     ausgefallenes Warm-up. */
  function warmupNoetig() {
    profilAbgleichen();
    if (entwicklerfrei() || !konfiguriert()) return false;
    if (zustand.modus !== 'bewertung' || !zustand.warmup_pflicht) return false;
    return zustand.warmup_heute !== true;
  }

  /* Ergebnis eines Laufs melden. Die Datenbank entscheidet, ob es der
     gewertete Pflichtlauf des Tages ist — deshalb wird die Antwort
     zurückgegeben und der Zustand danach neu geholt. */
  async function warmupMelden(nutzlast) {
    profilAbgleichen();
    if (!student() || !konfiguriert() || entwicklerfrei()) return null;
    const antwort = await rpc('chemie710_warmup_melden', nutzlast);
    /* Der Stand hat sich gerade geändert: „warmup_heute" ist jetzt wahr.
       Ohne diese Auffrischung stünde das Kind bis zur nächsten turnusmäßigen
       Prüfung vor derselben Aufforderung. */
    await aktualisieren({ erzwingen: true });
    return antwort;
  }

  /* Seit v3 gibt es je Lernweg ein eigenes Blatt. Hier ist die Einheit
  ...
  /* Die Aufforderung zum Warm-up. Bewusst kein Verbot, sondern ein Weg:
     Der Knopf führt direkt hin und kommt danach hierher zurück. */
  function warmupAufforderungAnzeigen(einheit, ziel) {
    const kasten = document.createElement('div');
    kasten.className = 'lernsperre lernsperre-warmup';
    kasten.setAttribute('role', 'status');

    const h = document.createElement('h2');
    h.textContent = 'Erst aufwärmen';
    const p = document.createElement('p');
    p.textContent = 'Im Unterricht beginnt jede Stunde mit fünf Aufgaben aus dem, '
      + 'was in den zurückliegenden Reihen dran war. Das dauert ein paar Minuten '
      + 'und zählt für die Bewertung.';
    const p2 = document.createElement('p');
    p2.className = 'lernsperre-klein';
    p2.textContent = 'Gewertet wird der erste Lauf des Tages. Nach dem Unterricht '
      + 'kannst du so oft üben, wie du willst — dann zählt nichts davon.';

    const los = document.createElement('a');
    los.className = 'btn btn-haupt';
    los.href = 'warmup.html' + (einheit ? '?u=' + encodeURIComponent(einheit) : '');
    los.textContent = 'Warm-up starten';

    const zurueck = document.createElement('a');
    zurueck.className = 'btn btn-neben';
    zurueck.href = 'index.html';
    zurueck.textContent = 'Zur Übersicht';

    const knoepfe = document.createElement('div');
    knoepfe.className = 'aktionen';
    knoepfe.append(los, zurueck);
    kasten.append(h, p, p2, knoepfe);
    (ziel || document.querySelector('main') || document.body).replaceChildren(kasten);
  }
  ...
  async function melden() {
    ...
    for (const [unit, offen] of Object.entries(snapshot)) {
      ...
      if (menge < 5) continue;
      try {
        await rpc('chemie710_lernzeit_melden', { p_unit: unit, p_sekunden: menge });
        ...
        console.warn('[Chemie710 Lernzeit] Meldung fehlgeschlagen:', fehler.message);
        break;
      }
    }
  }
  ...
  function starten(einheit) {
    ...
    addEventListener('chemie710:student-ready', () => {
      profilAbgleichen({ erzwingen: true });
