/* ============================================================
   ...
   das gesamte Menü auf jeder Seite.
...
(() => {
  ...
  const BOOT = window.Chemie710DevTools;
  if (!BOOT) {
    console.warn('[Chemie710 DEV] dev-boot.js fehlt — das Entwicklermenü bleibt aus.');
    return;
  }
  ...
  const FALLBACK_AREAS = [
    ...
    {
      code: 'qb',
      title: 'Quantitative Betrachtungen',
      anchor: 'bereich-qb',
      count: 5
    },
    {
    ...
    {
      code: 'os',
      title: 'Organische Säuren',
      anchor: 'bereich-os',
      count: 5
    },
    {
      code: 'es',
      title: 'Ester und Makromoleküle',
      anchor: 'bereich-es',
      count: 6
    }
  ];
  ...
  async function loadAreas() {
    try {
      ...
      console.warn('[Chemie710 Dev-Navigation] units/index.json:', error);
  ...
  function currentAreaCode() {
    ...
    const hashMatch = location.hash.match(/^#(?:bereich-)?(fc|ps|ga|wa|sz|me|qb|sl|kw|al|os|es)$/i);
    if (hashMatch) return hashMatch[1].toLowerCase();

    return readText('navArea', 'fc').toLowerCase();
  }
  ...
  async function clearOfflineCache() {
    if ('caches' in window) {
      ...
      await Promise.all(names.filter(name => name.startsWith('chemie710-')).map(name => caches.delete(name)));
    }
  }
  ...
  async function diagnoseSammeln() {
    ...
    let angemeldet = 'nein';
    try {
      const s = JSON.parse(localStorage.getItem('chemie710.student') || 'null');
      angemeldet = s?.student_id ? 'ja' : 'nein';
    } catch { /* egal */ }
    let diagnose = {};
    try { diagnose = window.CHEMIE710_DIAGNOSE_STATE?.() || {}; } catch { diagnose = {}; }
    ...
    return [
      'Chemie 7–10 · Diagnosebericht',
      'erstellt: ' + new Date().toLocaleString('de-DE'),
      '',
      '--- Anwendung ---',
      'Seite:            ' + location.pathname,
      'Einheit:          ' + (q.get('u') || '—'),
      'Pfad:             ' + (localStorage.getItem('chemie710.pfad') || '—').replace(/"/g, ''),
      ...
      'devMode:          ' + String(!!(window.CHEMIE710_SUPABASE && window.CHEMIE710_SUPABASE.devMode)),
      '',
      '--- Betrieb ---',
  ...
  function diagnoseSpeichern(text) {
    const datei = 'chemie710-diagnose-' + new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-') + '.txt';
  ...
  function buildMenu() {
    ...
    (async () => {
      ...
      try {
        if ('caches' in window) {
          aktiverCache = (await caches.keys()).filter(n => n.startsWith('chemie710-')).join(', ');
        }
  ...
  /* Die Einstellungen hat dev-boot.js bereits angewandt, und
     window.Chemie710DevTools steht dort. Hier bleibt nur der Aufbau. */
  if (!enabled) return;
