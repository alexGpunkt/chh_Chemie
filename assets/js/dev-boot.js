/* ============================================================
   ...
   CHANGELOG (v3) und nicht hier.

   Die eine Sache, die man wissen muss, bevor man etwas verschiebt:
   Was CONFIG ändert oder window.CHEMIE710_STUDENT setzt, MUSS hier stehen
   und synchron laufen. student-login.js ruft beim Laden
   Chemie710DevTools.saveTestStudent() auf; tracker.js und lernmodus.js
   lesen CONFIG.skipStudentLogin und CONFIG.devTrackerDisabled ebenfalls
   beim Laden. Nachgeladen käme das zu spät.

   Das Menü selbst (dev-tools.js) baut erst auf DOMContentLoaded auf und
   wird deshalb unten nachgeholt — nur bei devMode. Auf master fordert es
   niemand an.
   ============================================================ */
...
(() => {
  ...
  const PREFIX = 'chemie710.dev.';
  const STUDENT_KEY = 'chemie710.student';
  ...
  function saveTestStudent(key) {
    ...
    localStorage.setItem('chemie710.name', student.display_name);
    localStorage.setItem('chemie710.student_id', student.student_id);
    localStorage.setItem('chemie710.class_code', student.class_code);
    window.CHEMIE710_STUDENT = student;
    return student;
  }
  ...
  menue.addEventListener('error', () => {
    console.warn('[Chemie710 DEV] dev-tools.js nicht ladbar — Einstellungen gelten, Menü fehlt.');
  });
