...
'use strict';
const C = window.CHEMIE710_SUPABASE || {};
const $ = s => document.querySelector(s);
const AUTH_KEY = 'chemie710.teacher.session';
...
async function istFreigeschalteteLehrkraft(){
  const wert=await request(rest()+'rpc/chemie710_ist_lehrkraft',{method:'POST',body:'{}'});
  return wert===true;
}
...
async function loadModus(){
  try{
    const zeilen=await request(rest()+'chemie710_unterricht?select=*&id=eq.1');
...
async function setzeModus(modus){
  ...
  try{
    await request(rest()+'rpc/chemie710_unterricht_setzen',{method:'POST',
...
async function loadFreigaben(progress,events){
  ...
  let freigaben=[];
  try{ freigaben=await request(rest()+'chemie710_freigaben?select=*')||[]; }
  catch(e){
...
async function freigabeKlick(event){
  ...
  try{
    if(frei){
      await request(rest()+'rpc/chemie710_freigeben',{method:'POST',
      ...
      await request(rest()+'rpc/chemie710_freigabe_zuruecknehmen',{method:'POST',
...
async function loadLernzeit(){
  ...
  try{
    ...
    zeilen=await request(rest()+'chemie710_lernzeit?select=*&datum=gte.'+seit)||[];
  }catch(e){
  ...
  const namen=new Map();
  (await request(rest()+'chemie710_students?select=id,display_name').catch(()=>[])||[])
...
/* ---------- Warm-up und Bewertung  (V31) ----------
   Das Warm-up ist im Unterricht Pflicht und liefert damit als einziger Teil
   der Anwendung eine belastbare, regelmäßige Leistungsmessung: immer fünf
   Aufgaben, immer aus dem, was zurückliegt, immer unter denselben
   Bedingungen. Gewertet wird der erste Pflichtlauf eines Tages — sonst
   ...
   Zwei Wege zur Note, und die Wahl trifft die Lehrkraft je Kind:
     ...
     Lernfortschritt Der Stand am Ende, zuzüglich der Verbesserung gegenüber
                     dem Anfang. Für Kinder, bei denen der Mittelwert die
                     Entwicklung verdeckt statt sie zu zeigen.

   Gerechnet wird beides in der Datenbank (chemie710_warmup_uebersicht).
   Eine Notenskala gehört an eine Stelle und nicht in jeden Client — sonst
   steht in einem halben Jahr in zwei Dateien eine andere. */
async function loadWarmup(){
  const koerper=$('#warmupBody');
  if(!koerper)return;
  const cls=$('#classFilter').value.trim();
  const tage=Number($('#warmupTage')?.value)||180;
  let zeilen=[];
  try{
    zeilen=await request(rest()+'rpc/chemie710_warmup_uebersicht',{method:'POST',
      body:JSON.stringify({p_klasse:cls||null,p_tage:tage})})||[];
  }catch(e){
    koerper.innerHTML='<tr><td colspan="8" class="muted">Bewertungsübersicht nicht abrufbar — supabase/setup.sql erneut ausführen.</td></tr>';
    if($('#warmupAnzeige'))$('#warmupAnzeige').textContent='nicht verfügbar';
    return;
  }

  const mitLauf=zeilen.filter(z=>(z.laeufe||0)>0);
  if($('#warmupAnzeige')){
    $('#warmupAnzeige').textContent=mitLauf.length
      ? `${mitLauf.length} von ${zeilen.length} mit gewerteten Läufen`
      : 'noch keine gewerteten Läufe';
  }
  ...
  const entwicklung=z=>{
    ...
    const zeichen=d>0?'+':'';
    return `${prozent(z.quote_frueh)} → ${prozent(z.quote_spaet)} <span class="${d>0?'good':(d<0?'bad':'')}">(${zeichen}${d})</span>`;
  };
  ...
  koerper.innerHTML=zeilen.length?zeilen.map(z=>{
    ...
    const schwach=(z.schwache_kategorien||[]).length
      ? (z.schwache_kategorien||[]).map(k=>`<span class="badge">${esc(k)}</span>`).join(' ')
      : '–';
    return `<tr><td>${esc(z.name)}</td><td>${wahl}</td><td>${z.laeufe||0}</td>
      <td>${prozent(z.quote_gesamt)}</td><td>${entwicklung(z)}</td>
      <td>${note}</td><td>${z.letzter_lauf?esc(z.letzter_lauf):'–'}</td><td>${schwach}</td></tr>`;
  }).join(''):'<tr><td colspan="8" class="muted">Keine Lernenden in dieser Lerngruppe.</td></tr>';
}

async function bewertungsartAendern(event){
  const feld=event.target.closest('[data-bewertung]');
  if(!feld)return;
  const m=$('#warmupMeldung');
  try{
    await request(rest()+'rpc/chemie710_bewertungsart_setzen',{method:'POST',
...
async function loadOps(){
  ...
  let hinweis='';
  try{
    const zeilen=await request(rest()+'rpc/chemie710_wartung_status',{method:'POST',body:'{}'});
    const w=Array.isArray(zeilen)?zeilen[0]:zeilen;
    if(!w||!w.letzter_lauf){
      kacheln.push(['Aufräumjob','noch nie gelaufen','warn']);
      hinweis='chemie710_aufraeumen() wurde bisher nicht ausgeführt. Ohne Aufruf löscht nichts — siehe DATENSCHUTZ.md.';
    }else{
    ...
    hinweis='chemie710_wartung_status() fehlt in der Datenbank oder das Konto ist nicht freigeschaltet. supabase/setup.sql erneut ausführen.';
  }
...
async function loadRoster(){const cls=$('#classFilter').value.trim();const q='chemie710_students?select=id,login_name,display_name,class_code,active,created_at'+(cls?'&class_code=eq.'+encodeURIComponent(cls):'')+'&order=class_code,login_name';const rows=await request(rest()+q);renderRoster(rows||[])}
...
async function addStudent(event){event.preventDefault();const login=normalizeLogin($('#studentLogin').value),display=$('#studentDisplay').value.trim(),cls=$('#studentClass').value.trim();if(!validLogin(login)){rosterMessage('Benutzername muss nachname.vorname entsprechen.',true);return}try{await request(rest()+'chemie710_students',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({login_name:login,display_name:display,class_code:cls,active:true})});event.target.reset();$('#studentClass').value=$('#classFilter').value.trim();rosterMessage('Schüler wurde freigeschaltet.');await loadRoster()}catch(e){rosterMessage('Hinzufügen fehlgeschlagen: '+e.message,true)}}
async function rosterClick(event){const toggle=event.target.closest('[data-toggle]'),del=event.target.closest('[data-delete]');try{if(toggle){const active=toggle.dataset.active==='true';await request(rest()+'chemie710_students?id=eq.'+encodeURIComponent(toggle.dataset.toggle),{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({active:!active})});rosterMessage(active?'Zugang wurde gesperrt.':'Zugang wurde freigegeben.')}else if(del){if(!confirm('Diesen Eintrag wirklich löschen?'))return;await request(rest()+'chemie710_students?id=eq.'+encodeURIComponent(del.dataset.delete),{method:'DELETE',headers:{Prefer:'return=minimal'}});rosterMessage('Eintrag wurde gelöscht.')}else return;await loadRoster()}catch(e){rosterMessage('Änderung fehlgeschlagen: '+e.message,true)}}

async function loadDashboard(){const mins=Number($('#range').value),since=new Date(Date.now()-mins*60000).toISOString(),cls=$('#classFilter').value.trim(),cq=cls?'&class_code=eq.'+encodeURIComponent(cls):'';const [progress,events]=await Promise.all([request(rest()+'chemie710_progress?select=*&updated_at=gte.'+encodeURIComponent(since)+cq+'&order=updated_at.desc&limit=500'),request(rest()+'chemie710_events?select=*&ts=gte.'+encodeURIComponent(since)+cq+'&order=ts.desc&limit=2000')]);render(progress||[],events||[]);loadFreigaben(progress||[],events||[]);loadLernzeit();loadWarmup()}
...
/* ============================================================
   ...
   einmal ein.
   ============================================================ */
const BEAMER_KANAL='chemie710-beamer';
const BEAMER_SPEICHER='chemie710.beamer.einstellungen';
...
function beamerZeilenBauen(progress,events){
  ...
  /* Zwei Zeitreihen statt einer: JEDES erfolgreich angekommene Ereignis
     ...
     jüngsten Heartbeat. Mit einer einzigen Reihe würde er auf 0 fallen,
     sobald nach einem Heartbeat z. B. eine Antwort neuer ist. */
...
function beamerOeffnen(){
  ...
  beamerFenster=window.open('beamer.html','chemie710-beamer');
  if(!beamerFenster){
...
/* ---------- Kopiervorlagen aus dem Eduki-Bestand ----------
   Die Liste kommt aus eduki-material.json, die Dateien selbst bleiben auf
   dem Rechner der Lehrkraft. Das ist keine Bequemlichkeit, sondern die
   Lizenzlage: Eduki-Material darf in der eigenen Lerngruppe eingesetzt,
   aber nicht weitergegeben werden — und alles im Repository landet mit dem
   nächsten Push öffentlich auf GitHub Pages. Verwiesen wird deshalb auf den
   Pfad, gedruckt wird von dort.

   Läuft ohne Supabase: Die Tafel ist Unterrichtsvorbereitung und soll auch
   dann stehen, wenn die Verbindung fehlt. */
let edukiDaten=null;
async function loadEduki(){
  const wahl=$('#edukiEinheit');if(!wahl)return;
  try{
    const antwort=await fetch('eduki-material.json',{cache:'no-cache'});
    if(!antwort.ok)throw new Error('HTTP '+antwort.status);
    edukiDaten=await antwort.json();
  }catch(e){
    $('#edukiBody').innerHTML='<tr><td colspan="4">Liste nicht gefunden — '
      +'<code>python Eduki/zuordnung_projekt.py</code> erzeugt sie neu.</td></tr>';
    return;
  }
  const ids=Object.keys(edukiDaten.einheiten).sort();
  wahl.innerHTML=ids.map(id=>`<option value="${esc(id)}">${esc(id.toUpperCase())}</option>`).join('');
  edukiZeigen();
}
function edukiZeigen(){
  if(!edukiDaten)return;
  const id=$('#edukiEinheit').value;
  const liste=edukiDaten.einheiten[id]||[];
  $('#edukiBody').innerHTML=liste.length?liste.map(m=>`<tr>
    <td>${esc(m.paket.replace(/__[0-9a-f]{6,}$/,''))}</td>
    <td>${esc(m.art)}</td><td>${esc(m.stufen)}</td><td>${esc(m.hinweis)}</td></tr>`).join('')
    :'<tr><td colspan="4">Für diese Einheit ist kein Eduki-Material zugeordnet.</td></tr>';
  const pfad=$('#edukiPfad');
  pfad.textContent=liste.length?('Liegt unter '+edukiDaten.wurzel+' — von dort drucken, nicht ins Repository kopieren.'):'';
  pfad.hidden=!liste.length;
}
$('#edukiEinheit')?.addEventListener('change',edukiZeigen);
loadEduki();
